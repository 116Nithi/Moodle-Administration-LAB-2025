...
**บันทึกผลการทดลอง 1:**
```bash <img width="752" height="471" alt="image" src="https://github.com/user-attachments/assets/e9a7a286-baac-445b-80a8-0507190ee121" />
...
```bash
<img width="940" height="630" alt="image" src="https://github.com/user-attachments/assets/f2ae9d1d-b01c-4061-b066-bd1a067a8ae7" />
...
**<img width="909" height="500" alt="image" src="https://github.com/user-attachments/assets/3bee8641-7989-424a-b2fb-008aa214f434" />
**
```
...
คำตอบ: 
```
 Docker Image = แม่แบบ / ต้นฉบับ
Docker Container = ตัวที่ถูกสร้างจาก Image และกำลังทำงาน
เชื่อมโยงกับ Moodle + MariaDB
เปรียบเทียบ Docker vs Virtual Machine
เขียนคำตอบแบบ เชิงวิชาการส่งรายงาน
```
...
คำตอบ: 
```
มีทั้งหมด 2 Containers Container: Database (MariaDB) Image ที่ใช้: mariadb

หน้าที่:
เก็บข้อมูลทั้งหมดของระบบ Moodle เช่น
ข้อมูลผู้ใช้ (Users)
รายวิชา (Courses)
คะแนน (Grades)
Cohort และการตั้งค่าต่าง ๆ
ลักษณะการทำงาน:
ทำงานเป็นฐานข้อมูลกลาง
Moodle จะเชื่อมต่อเข้ามาเพื่ออ่าน/บันทึกข้อมูล
สำคัญ: ถ้า Container นี้หยุด → Moodle จะใช้งานไม่ได้

Container: Moodle Application
Image ที่ใช้: moodle (เช่น lthub/moodle)
หน้าที่:
แสดงเว็บไซต์ Moodle ให้ผู้ใช้ใช้งานผ่านเว็บเบราว์เซอร์
จัดการการเรียนการสอน เช่น
Upload Users
จัดการ Course
กิจกรรมและการประเมินผล
ลักษณะการทำงาน:
ติดต่อกับ Container ฐานข้อมูลเพื่อดึง/บันทึกข้อมูล
เปิดพอร์ตให้ผู้ใช้เข้าถึงผ่านเว็บ
```
...
```
Named Volume
เป็น Volume ที่ Docker จัดการตำแหน่งจัดเก็บให้โดยอัตโนมัติ
ไม่ผูกกับโฟลเดอร์ในเครื่องโดยตรง

ข้อดีของการใช้ Named Volume
ข้อมูลไม่หายเมื่อ Container ถูกลบหรือสร้างใหม่
แยกข้อมูลออกจากตัว Container ชัดเจน
จัดการง่าย เหมาะกับผู้เริ่มต้น
ปลอดภัยกว่าการผูกโฟลเดอร์ตรง (Bind Mount)
รองรับการสำรองข้อมูล (Backup) ได้สะดวก

ข้อเสียของการใช้ Named Volume
ไม่เห็นตำแหน่งไฟล์ชัดเจนในเครื่องโฮสต์
แก้ไขไฟล์จากภายนอกโดยตรงทำได้ยาก
ต้องใช้คำสั่ง Docker ในการจัดการ เช่น
```
...
คำตอบ:
```
Container สามารถเรียกหากันได้โดยใช้ ชื่อ service ที่กำหนดไว้ใน docker-compose.yml
โดยไม่จำเป็นต้องรู้ IP address
Docker จะสร้าง DNS ภายใน Network
ทำให้ Container สามารถแปลงชื่อ service → IP ได้อัตโนมัติ
```
...
คำตอบ:
```
กำหนดลำดับการเริ่มต้น (startup order) ของ Container
เพื่อให้ Container หนึ่งเริ่มทำงาน ก่อน อีก Container
ทำไม depends_on ถึงสำคัญ
ลดปัญหาแอปเริ่มก่อนบริการที่ต้องใช้
Moodle ต้องเชื่อมต่อฐานข้อมูล
ถ้า db ยังไม่เริ่ม → ระบบจะ error
จัดการระบบหลาย Container ได้เป็นระเบียบ
กำหนดความสัมพันธ์ระหว่างบริการชัดเจน
ทำให้การรันระบบเสถียรมากขึ้น
โดยเฉพาะระบบที่มี Database / Cache / API
```
...
คำตอบ:
```
แก้ไขบรรทัด ports จากเดิม เช่น
ports:
  - "80:80"
เป็น
ports:
  - "9000:80"
9000 → พอร์ตฝั่งเครื่องโฮสต์ (ที่เราเรียกผ่านเบราว์เซอร์)
80 → พอร์ตภายใน Container (Moodle ใช้งานอยู่)
```
...
MOODLE_DB_HOST=db
หมายความว่า
กำหนดให้ Moodle เชื่อมต่อฐานข้อมูลที่ host ชื่อว่า db
โดย db คือ
ชื่อ service ของ Container ฐานข้อมูล
ที่กำหนดไว้ในไฟล์ docker-compose.yml
ตัวอย่าง:
services:
  db:
    image: mariadb

แล้วทำไมไม่ใช้ localhost
ใน Docker:
localhost หมายถึง Container ตัวเอง
ไม่ได้หมายถึงเครื่อง Host
และไม่ใช่ Container อื่น
ดังนั้นถ้าเขียน:
MOODLE_DB_HOST=localhost


จะหมายความว่า
➡️ Moodle พยายามหา Database ใน Container ของตัวเอง
➡️ ซึ่งไม่มี MariaDB
➡️ ผลลัพธ์คือ เชื่อมต่อฐานข้อมูลไม่ได้

➡️ Docker จะสร้าง DNS ภายใน
➡️ แปลงชื่อ db ให้เป็น IP ของ Container ฐานข้อมูลอัตโนมัติ
```
...
คำตอบ:
```
ข้อดี/ข้อเสียการติดตั้ง Moodle ด้วย Docker เทียบกับการติดตั้งแบบปกติ

Docker (ติดตั้งด้วย Docker Compose)
ข้อดี
- ติดตั้งเร็วและทำซ้ำได้เหมือนกันทุกเครื่อง (ย้ายเครื่อง/ย้ายเซิร์ฟเวอร์ง่าย)
- แยกส่วนบริการชัดเจน (Moodle / DB / Redis ฯลฯ) ลดการชนกันของเวอร์ชัน PHP, DB
- อัปเดต/ย้อนเวอร์ชันง่าย (เปลี่ยน image tag แล้ว up ใหม่)
- ลบและสร้างใหม่ได้โดยไม่ทำให้ข้อมูลหาย ถ้าใช้ Volume ถูกต้อง

ข้อเสีย
- ต้องเข้าใจแนวคิด Docker/Compose, network, volume เพิ่มเติม
- การดีบักบางอย่างซับซ้อนกว่า (ดู log/เข้าคอนเทนเนอร์/สิทธิ์ไฟล์)
- การปรับแต่งระดับระบบ (เช่น PHP extension/OS package) ต้องทำผ่าน Dockerfile หรือ image ที่รองรับ
- ถ้าตั้งค่า volume/permission ไม่ดี อาจเจอปัญหาเขียนไฟล์ไม่ได้
...
services:
  db:
    ...
    container_name: moodle_db
    environment:
      MYSQL_ROOT_PASSWORD: AsPpeez1875!
      MYSQL_DATABASE: moodle
      MYSQL_USER: moodleuser
      MYSQL_PASSWORD: AsPpeez1875
    volumes:
  ...
  moodle:
    ...
    environment:
      ...
      MOODLE_DB_NAME: moodle
      MOODLE_DB_USER: moodleuser
      MOODLE_DB_PASSWORD: AsPpeez1875
      MOODLE_URL: http://localhost/
      # หมายเหตุ: ตัวแปร Redis ของ image แต่ละเจ้าอาจไม่เหมือนกัน
      # ถ้า image นี้รองรับ env สำหรับ Redis ให้ใส่เพิ่ม เช่น:
...
- docker compose ps
ดูว่า db และ moodle_app ขึ้น (Up) หรือ db รีสตาร์ท/Exited

2) ดู Log ของ Database และ Moodle
- docker compose logs -f db
- docker compose logs -f moodle
สังเกต error เช่น “Access denied”, “Unknown database”, “Could not connect”, “Name or service not known”

3) ทดสอบว่า moodle_app มองเห็น db ผ่าน network ได้จริงไหม
- docker compose exec moodle ping -c 3 db
หรือ
- docker compose exec moodle getent hosts db
ถ้าไม่เจอ แปลว่า network/ชื่อ service ผิด หรือไม่ได้อยู่ network เดียวกัน

4) ทดสอบการเชื่อมต่อพอร์ต DB จากฝั่ง moodle_app
(ถ้ามีเครื่องมือใน container)
- docker compose exec moodle sh -lc "nc -zv db 3306"
ถ้าไม่มี nc ให้ดูแค่ ping/getent + logs ก็พอ

5) ตรวจสอบค่า environment ที่เกี่ยวกับ DB ใน docker-compose.yml
- MOODLE_DB_HOST ต้องเป็น “db” (ชื่อ service ฐานข้อมูล)
- MOODLE_DB_NAME ต้องตรงกับ MYSQL_DATABASE
- MOODLE_DB_USER / MOODLE_DB_PASSWORD ต้องตรงกับ MYSQL_USER / MYSQL_PASSWORD

6) ตรวจสอบว่า DB พร้อมใช้งานหรือยัง (db เริ่มช้า)
- ดูใน logs ของ db ว่ามีข้อความประมาณ “ready for connections” แล้วหรือยัง
เพราะ depends_on ช่วยแค่ “สั่งเริ่ม” ไม่ได้รอให้ DB ready

วิธีแก้ไข:
1) แก้ค่าชื่อฐานข้อมูล/ผู้ใช้/รหัสผ่านให้ตรงกัน
ตัวอย่างให้ “ตรงชุดเดียวกัน”:
- db:
  MYSQL_DATABASE=moodle
  MYSQL_USER=moodleuser
  MYSQL_PASSWORD=AsPpeez1875
...
2) แก้ปัญหา network
- ให้ทั้ง db และ moodle อยู่ใน network เดียวกัน (เช่น moodle_network)
- ตรวจว่าใช้ชื่อ service “db” ไม่ใช่ชื่อ container/localhost

3) รีสตาร์ทระบบหลังแก้ compose
- docker compose down
- docker compose up -d

4) ถ้าเป็นปัญหา DB ยังไม่ ready ให้เพิ่ม healthcheck + depends_on แบบรอสุขภาพ (แนะนำ)
- เพิ่ม healthcheck ให้ db แล้วให้ moodle depends_on แบบ condition: service_healthy
(ช่วยลดโอกาส Moodle เริ่มแล้วต่อ DB ไม่ทัน)

5) ถ้าเคยสร้าง db_data ผิดพลาด (เช่นสร้าง DB ด้วยชื่ออื่น) อาจต้องล้างข้อมูลแล้วเริ่มใหม่
- docker compose down -v  (ระวังข้อมูลหาย)
แล้ว up ใหม่เพื่อให้สร้างฐานข้อมูลใหม่ตาม MYSQL_DATABASE
```
...
คำตอบ:
```
ถ้ารัน docker-compose down -v จะเกิดอะไรขึ้นกับข้อมูล?
docker-compose down จะหยุดและลบ Containers, Networks (ที่ compose สร้าง) และ default network
เมื่อเพิ่ม -v จะ “ลบ Volume ด้วย” ทั้ง named volumes และ anonymous volumes ที่ผูกกับ services ใน compose นั้น
ผลที่เกิดกับข้อมูล:
ข้อมูลใน Volume จะถูกลบถาวร เช่น
db_data (ข้อมูล MariaDB ทั้งหมด: ผู้ใช้ คอร์ส คะแนน)
moodledata (ไฟล์อัปโหลด/ไฟล์รายวิชา/ข้อมูล moodledata)
redis_data (ถ้ามี)
ทำให้เปิดขึ้นมาใหม่จะเหมือน “ติดตั้งใหม่หมด” ต้องตั้งค่าและสร้างข้อมูลใหม่ทั้งหมด
